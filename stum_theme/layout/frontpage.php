 ...
 *
 */
$stum_body_attributes = 'has-region-side-pre has-region-side-post';
$hassidepre = $PAGE->blocks->region_has_content('side-pre', $OUTPUT);
if ($hassidepre) {$stum_body_attributes .= ' used-region-side-pre';} else {$stum_body_attributes .= ' empty-region-side-pre';}
$hassidepost = $PAGE->blocks->region_has_content('side-post', $OUTPUT);
if ($hassidepost) {$stum_body_attributes .= ' used-region-side-post';} else {$stum_body_attributes .= ' empty-region-side-post';}
$blockstyle = theme_stum_get_setting('block_style');
if ($blockstyle == 0) {$stum_body_attributes .= ' blockstyle-01';}
if ($blockstyle == 1) {$stum_body_attributes .= ' blockstyle-02';}
if ($blockstyle == 2) {$stum_body_attributes .= ' blockstyle-03';}
...
if ($sidebar) {theme_stum_init_sidebar($PAGE); $sidebar_stat = theme_stum_get_sidebar_stat(); $stum_body_attributes .= ' sidebar-enabled '.$sidebar_stat;}
...
<head>
    ...
    <noscript>
			<link rel="stylesheet" type="text/css" href="<?php echo $CFG->wwwroot;?>/theme/stum/style/nojs.css" />
	</noscript>
    <!-- Google web fonts -->
    <?php require_once(dirname(__FILE__).'/includes/fonts.php'); ?>
</head>

<body <?php echo $OUTPUT->body_attributes("$stum_body_attributes"); ?>>
...
<div id="page" class="container-fluid">
	...
	<div class="hidden-blocks">
    	<div class="row-fluid">
        	<h4><?php echo get_string('visibleadminonly', 'theme_stum') ?></h4>
            <?php
	...
	<a href="#top" class="back-to-top"><span class="stum-sr-only"><?php echo get_string('back'); ?></span></a>
...
<?php echo $OUTPUT->stum_footer_scripts(); ?>
<?php if ($hasslideshow) {echo $OUTPUT->stum_fp_slideshow();} ?>
<?php if ($hascarousel) {echo $OUTPUT->stum_fp_carousel();} ?>
