...
<div class="container-fluid">    
	...
	if (empty($CFG->loginhttps)) {
		...
		if (!isloggedin() or isguestuser()) {
			...
			else if ($login_link=='3') {$login_link_url = $wwwroot.'/login/index.php'; $login_link_txt = get_string('moodle_login_page','theme_stum');}
			...
			if ($auth_googleoauth2) {
				...
				<form class="navbar-form pull-right" method="post" action="<?php p($wwwroot) ?>/login/index.php">
						<div id="block-login">
							...
							<label for="inputName" class="stum-sr-only"><?php echo $username; ?></label>
							...
							<input id="inputName" class="span2" type="text" name="username" placeholder="<?php echo $username; ?>">
								...
								<label for="inputPassword" class="stum-sr-only"><?php echo get_string('password'); ?></label>
								<input id="inputPassword" class="span2" type="password" name="password" placeholder="<?php echo get_string('password'); ?>">
									<button type="submit" id="submit"><span class="stum-sr-only"><?php echo get_string('login'); ?></span></button>
								</div>
