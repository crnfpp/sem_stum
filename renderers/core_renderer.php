 ...
 */
 ...
 class theme_stum_core_renderer extends core_renderer {
	...
	protected function render_custom_menu(custom_menu $menu) {
		...
		$hasdisplaymycourses = theme_stum_get_setting('mycourses_dropdown');
	...
	public function stum_fp_slideshow() {
		...
		$loader='';
		if(theme_stum_get_setting('slideshow_loader')==0) {$loader='bar';}
		else if(theme_stum_get_setting('slideshow_loader')==1) {$loader='pie';}
		else if(theme_stum_get_setting('slideshow_loader')==3) {$loader='none';}
		
		$imgfx='random';
		if (theme_stum_get_setting('slideshow_imgfx')!='') {$imgfx=theme_stum_get_setting('slideshow_imgfx');}
		
		$slideshow_height='auto';
		if(theme_stum_get_setting('slideshow_height')=='responsive') {			
			if (!empty(theme_stum_get_setting('slide1image'))) {
				$context = context_system::instance();
				$filename = $theme->settings->slide1image;
				$fs = get_file_storage();
				$file = $fs->get_file($context->id, 'theme_stum', 'slide1image', 0, '/', $filename);
		...
		$advance='true';
		if(theme_stum_get_setting('slideshow_advance')==0) {$advance='false';}
		
		$navhover='true';
		if(theme_stum_get_setting('slideshow_nav')==0) {$navhover='false';}
	...
	public function stum_fp_carousel() {
		$tag = "script";
		$html = "";
		
		$carousel_img_dim = theme_stum_get_setting('carousel_img_dim');
	...
	public function stum_h5p_header() {
		$header = new stdClass();
		$header->contextheader = $this->context_header();
		$header->headeractions = $this->page->get_header_actions();
		return $this->render_from_template('theme_stum/stum_h5p_header',$header);
	}
